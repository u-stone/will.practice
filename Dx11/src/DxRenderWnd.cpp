...
DxRenderWnd::DxRenderWnd(HINSTANCE hInst, int width, int height)
    : RenderWnd(hInst, width, height),
      vertex_shader_file_(L"shader/vertex_shader.hlsl"),
      vertex_entry_("VS"),
      pixel_shader_file_(L"shader/pixel_shader.hlsl"),
      pixel_entry_("PS") {}

DxRenderWnd::~DxRenderWnd() {
  DxPtr<ID3D11Debug> debugInterface;
  HRESULT hr = d3d11_device_->QueryInterface(__uuidof(ID3D11Debug),
                                             (void**)&debugInterface);
  if (SUCCEEDED(hr)) {
...
void DxRenderWnd::InitDevice() {
  ...
  sd.Windowed = TRUE;
  sd.SwapEffect =
      DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL;  // Options for handling pixels in a
                                         // display surface after calling
                                         // IDXGISwapChain1::Present1.
  sd.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH |
             DXGI_SWAP_CHAIN_FLAG_RESTRICTED_CONTENT;  // over win 8, for better
                                                       // performance.

  HRESULT hr = ::D3D11CreateDeviceAndSwapChain(
      adapter, driver_type, module, flags, feature_levels, feature_level_num,
      sdk_version, &sd, &swap_chain_, &d3d11_device_, &feature_level_selected_,
      &d3d11_context_);
  if (FAILED(hr)) {
    DxError(hr);
    return;
...
void DxRenderWnd::InitRenderView() {
  ...
  hr = d3d11_device_->CreateRenderTargetView(pBackBuffer.get(), NULL,
                                             &target_view_);
  if (FAILED(hr)) {
    DxError(hr);
    return;
...
void DxRenderWnd::LoadVertexShader() {
  DxPtr<ID3DBlob> blob =
      CreateShader(vertex_shader_file_, vertex_entry_, "vs_5_0");
  if (!blob) {
    return;
  }

  HRESULT hr = d3d11_device_->CreateVertexShader(
      blob->GetBufferPointer(), blob->GetBufferSize(), NULL, &vertex_shader_);
  if (FAILED(hr)) {
    DxError(hr);
    return;
  }
...
void DxRenderWnd::LoadPixelShader() {
  DxPtr<ID3DBlob> blob =
      CreateShader(pixel_shader_file_, pixel_entry_, "ps_5_0");
  if (!blob) {
    return;
  }
...
DxPtr<ID3DBlob> DxRenderWnd::CreateShader(std::wstring filename,
  ...
  HRESULT hr = D3DCompileFromFile(
      filename.c_str(), shader_macro, include, entry_point.c_str(),
      target.c_str(), shader_flag1, shader_flag2, &blob, &blob_result);
  if (FAILED(hr)) {
    DxError(hr);
    return nullptr;
  }
...
void DxRenderWnd::Render() {
  ...
  // (the screen) for flip mode, we must set a syncinterval for legcy mode, we
  // set syncinterval as 0
