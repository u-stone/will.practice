...
DxCubeRenderWndWithLights::DxCubeRenderWndWithLights(HINSTANCE hInst,
                                                     int width,
                                                     int height)
    : DxCubeRenderWnd(hInst, width, height) {
  vertex_shader_file_ = L"shader/cube_with_light.hlsl";
  vertex_entry_ = "VS";
  pixel_shader_file_ = L"shader/cube_with_light.hlsl";
  pixel_entry_ = "PS";
}
...
void DxCubeRenderWndWithLights::LoadPixelShader() {
  ...
  DxPtr<ID3DBlob> blob_solid =
      CreateShader(pixel_shader_file_, "PSSolid", "ps_5_0");
  if (!blob_solid) {
    return;
  }

  HRESULT hr = d3d11_device_->CreatePixelShader(blob_solid->GetBufferPointer(),
                                                blob_solid->GetBufferSize(),
                                                NULL, &pixel_shader_solid_);
  if (FAILED(hr)) {
    DxError(hr);
    return;
  }
...
void DxCubeRenderWndWithLights::Render() {
  ...
  XMFLOAT4 vLightColors[2] = {XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f),
  ...
  for (int m = 0; m < 2; ++m) {
    XMMATRIX mLight =
        XMMatrixTranslationFromVector(5.0f * XMLoadFloat4(&vLightDirs[m]));
